...
class MasterCmd(cmd.Cmd):
    ...
    def do_disable_unsol(self, line):
        ...
        self.application.master.PerformFunction(
            "disable unsolicited", opendnp3.FunctionCode.DISABLE_UNSOLICITED, headers, opendnp3.TaskConfig().Default()
        )
    ...
    def do_o1(self, line):
        ...
        self.application.send_direct_operate_command(
            opendnp3.ControlRelayOutputBlock(opendnp3.ControlCode.LATCH_ON), 5, command_callback
        )

    def do_o2(self, line):
        """Send a DirectOperate AnalogOutput (group 41) index 10 value 7 to the Outstation. Command syntax is: o2"""
        self.application.send_direct_operate_command(opendnp3.AnalogOutputInt32(7), 10, command_callback)

    def do_o3(self, line):
        """Send a DirectOperate BinaryOutput (group 12) CommandSet to the Outstation. Command syntax is: o3"""
        self.application.send_direct_operate_command_set(
            opendnp3.CommandSet(
                [
    ...
    def do_s1(self, line):
        ...
        self.application.send_select_and_operate_command(
            opendnp3.ControlRelayOutputBlock(opendnp3.ControlCode.LATCH_ON), 8, command_callback
        )

    def do_s2(self, line):
        """Send a SelectAndOperate BinaryOutput (group 12) CommandSet to the Outstation. Command syntax is: s2"""
        self.application.send_select_and_operate_command_set(
            opendnp3.CommandSet(
                [opendnp3.WithIndex(opendnp3.ControlRelayOutputBlock(opendnp3.ControlCode.LATCH_ON), 0)]
            ),
            command_callback,
    ...
    def do_write_time(self, line):
        ...
        self.application.master.Write(
            opendnp3.TimeAndInterval(opendnp3.DNPTime(millis_since_epoch), 100, opendnp3.IntervalUnits.Seconds),
            0,  # index
